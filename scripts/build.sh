#!/usr/bin/env bash
set -euo pipefail
# Load locked toolchain if available
...
umask 022
# Hybrid Strict tool resolution (major tools must be invoked via absolute paths).
# Runner should inject STUNIR_TOOL_ for every locked tool.
# In particular, this script relies on:
# - STUNIR_TOOL_PYTHON (required in strict mode)
# - STUNIR_TOOL_GIT (optional; only needed if build steps call git)
#
# In strict mode, we do NOT fall back to PATH lookup for major tools.
PY_BIN="${STUNIR_TOOL_PYTHON:-${PY_BIN:-}}"
if [[ -z "$PY_BIN" ]]; then
if [[ "${STUNIR_STRICT:-1}" == "1" ]]; then
echo "STUNIR_STRICT=1 requires STUNIR_TOOL_PYTHON to be set to a locked absolute path." 1>&2
exit 3
fi
PY_BIN="$(command -v python3 || command -v python || echo python3)"
fi
GIT_BIN="${STUNIR_TOOL_GIT:-${GIT_BIN:-}}"
if [[ -z "$GIT_BIN" ]]; then
if [[ "${STUNIR_STRICT:-1}" != "1" ]]; then
GIT_BIN="$(command -v git || echo git)"
fi
fi
# Compatibility shims for child bash scripts: ensure python3/python/git never resolve via PATH.
python3() { "$PY_BIN" "$@"; }
python() { "$PY_BIN" "$@"; }
export -f python3 python >/dev/null 2>&1 || true
if [[ -n "$GIT_BIN" ]]; then
git() { "$GIT_BIN" "$@"; }
export -f git >/dev/null 2>&1 || true
fi
# Policy knobs
...
CANONICAL_EPOCH="$("$PY_BIN" -c 'import json; print(json.load(open("build/epoch.json"))["selected_epoch"])')"
STUNIR_EPOCH_SOURCE="$("$PY_BIN" -c 'import json; print(json.load(open("build/epoch.json"))["source"])')"
else
"$PY_BIN" tools/epoch.py --out-json "$EPOCH_JSON" --print-epoch > build/.epoch_val
...
"$PY_BIN" tools/ir_in_source_ingest.py \
--repo-root . \
...
"$PY_BIN" tools/record_receipt.py \
...
"$PY_BIN" tools/spec_to_ir.py --spec-root spec --out asm/spec_ir.txt --epoch-json "$EPOCH_JSON"
# IR files now emitted as deterministic dCBOR bytes (*.dcbor) plus a simple offset bundle.
"$PY_BIN" tools/spec_to_ir_files.py --spec-root spec --out-root asm/ir --epoch-json "$EPOCH_JSON" --manifest-out receipts/ir_manifest.json --float-policy "$STUNIR_CBOR_FLOAT_POLICY" --bundle-out asm/ir_bundle.bin --bundle-manifest-out receipts/ir_bundle_manifest.json
# Generate provenance *after* IR emission so asm_digest commits to final asm/ state
"$PY_BIN" tools/gen_provenance.py --epoch "$STUNIR_BUILD_EPOCH" --spec-root spec --asm-root asm --out-header build/provenance.h --out-json build/provenance.json --epoch-source "$STUNIR_EPOCH_SOURCE"
# Record IR receipt (bind tool + argv; include generator sources as inputs)
"$PY_BIN" tools/record_receipt.py --target asm/spec_ir.txt --receipt receipts/spec_ir.json --status GENERATED_IR --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json tools/spec_to_ir.py tools/spec_to_ir_files.py tools/dcbor.py tools/gen_provenance.py tools/record_receipt.py --input-dirs spec asm --tool "$PY_BIN" --argv "$PY_BIN" tools/spec_to_ir.py --spec-root spec --out asm/spec_ir.txt --epoch-json "$EPOCH_JSON" --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
# Try compiling the provenance emitter for the host, if a C compiler exists
# Hybrid Strict: do not resolve compilers via PATH in strict mode.
CC_BIN="${CC:-}"
if [[ -z "$CC_BIN" ]]; then
if [[ -n "${STUNIR_TOOL_CLANG:-}" ]]; then
CC_BIN="${STUNIR_TOOL_CLANG}"
elif [[ -n "${STUNIR_TOOL_GCC:-}" ]]; then
CC_BIN="${STUNIR_TOOL_GCC}"
elif [[ "${STUNIR_STRICT:-1}" != "1" ]]; then
if command -v clang >/dev/null 2>&1; then CC_BIN=clang; fi
if [[ -z "$CC_BIN" ]] && command -v gcc >/dev/null 2>&1; then CC_BIN=gcc; fi
fi
fi
...
SBCL_BIN="$("$PY_BIN" tools/dep_receipt_tool.py --receipt receipts/deps/sbcl.json --require-accepted --print-path || true)"
fi
if [[ -z "$SBCL_BIN" ]]; then
...
"$PY_BIN" tools/ir_to_node.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/node/app
"$PY_BIN" tools/emit_output_manifest.py --root asm/node/app --manifest-out receipts/node_manifest.json
"$PY_BIN" tools/record_receipt.py --target receipts/node_manifest.json --receipt receipts/node.json --status CODEGEN_EMITTED_NODE --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/requirements.json tools/ir_to_node.py tools/emit_output_manifest.py tools/record_receipt.py --input-dirs spec asm --tool "$PY_BIN" --argv "$PY_BIN" tools/ir_to_node.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/node/app --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
NODE_BIN=""
if [[ -f receipts/deps/node_runtime.json ]]; then
NODE_BIN="$("$PY_BIN" tools/dep_receipt_tool.py --receipt receipts/deps/node_runtime.json --require-accepted --print-path || true)"
fi
if [[ -n "$NODE_BIN" ]]; then
"$NODE_BIN" asm/node/app/index.js > asm/node/app/run_stdout.json
"$PY_BIN" tools/record_receipt.py --target asm/node/app/run_stdout.json --receipt receipts/node_run.json --status RUNTIME_STDOUT_EMITTED_NODE --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/node_manifest.json receipts/deps/node_runtime.json asm/node/app/index.js asm/node/app/package.json tools/dep_receipt_tool.py tools/record_receipt.py --input-dirs spec asm --tool "$NODE_BIN" --argv "$NODE_BIN" asm/node/app/index.js --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
else
...
"$PY_BIN" tools/ir_to_java.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/java/app
"$PY_BIN" tools/emit_output_manifest.py --root asm/java/app --manifest-out receipts/java_manifest.json
"$PY_BIN" tools/record_receipt.py --target receipts/java_manifest.json --receipt receipts/java.json --status CODEGEN_EMITTED_JAVA --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/requirements.json tools/ir_to_java.py tools/emit_output_manifest.py tools/record_receipt.py --input-dirs spec asm --tool "$PY_BIN" --argv "$PY_BIN" tools/ir_to_java.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/java/app --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
fi
# ---------- .NET (hosted runtime) ----------
if has_target "dotnet" "${NORMALIZED_TARGETS}"; then
echo "Emitting dotnet (.NET hosted runtime)"
"$PY_BIN" tools/ir_to_dotnet.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/dotnet/app
"$PY_BIN" tools/emit_output_manifest.py --root asm/dotnet/app --manifest-out receipts/dotnet_manifest.json
"$PY_BIN" tools/record_receipt.py --target receipts/dotnet_manifest.json --receipt receipts/dotnet.json --status CODEGEN_EMITTED_DOTNET --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/requirements.json tools/ir_to_dotnet.py tools/emit_output_manifest.py tools/record_receipt.py --input-dirs spec asm --tool "$PY_BIN" --argv "$PY_BIN" tools/ir_to_dotnet.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/dotnet/app --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
fi
# ---------- Ruby (hosted runtime) ----------
if has_target "ruby" "${NORMALIZED_TARGETS}"; then
echo "Emitting ruby (hosted runtime)"
"$PY_BIN" tools/ir_to_ruby.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/ruby/app
"$PY_BIN" tools/emit_output_manifest.py --root asm/ruby/app --manifest-out receipts/ruby_manifest.json
"$PY_BIN" tools/record_receipt.py --target receipts/ruby_manifest.json --receipt receipts/ruby.json --status CODEGEN_EMITTED_RUBY --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/requirements.json tools/ir_to_ruby.py tools/emit_output_manifest.py tools/record_receipt.py --input-dirs spec asm --tool "$PY_BIN" --argv "$PY_BIN" tools/ir_to_ruby.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/ruby/app --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
RUBY_BIN=""
if [[ -f receipts/deps/ruby_runtime.json ]]; then
RUBY_BIN="$("$PY_BIN" tools/dep_receipt_tool.py --receipt receipts/deps/ruby_runtime.json --require-accepted --print-path || true)"
fi
if [[ -n "$RUBY_BIN" ]]; then
"$RUBY_BIN" asm/ruby/app/main.rb > asm/ruby/app/run_stdout.json
"$PY_BIN" tools/record_receipt.py --target asm/ruby/app/run_stdout.json --receipt receipts/ruby_run.json --status RUNTIME_STDOUT_EMITTED_RUBY --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/ruby_manifest.json receipts/deps/ruby_runtime.json asm/ruby/app/main.rb tools/dep_receipt_tool.py tools/record_receipt.py --input-dirs spec asm --tool "$RUBY_BIN" --argv "$RUBY_BIN" asm/ruby/app/main.rb --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
else
"$PY_BIN" tools/record_receipt.py --target asm/ruby/app/run_stdout.json --receipt receipts/ruby_run.json --status SKIPPED_TOOLCHAIN --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/ruby_manifest.json receipts/deps/ruby_runtime.json asm/ruby/app/main.rb tools/dep_receipt_tool.py tools/record_receipt.py --input-dirs spec asm --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
fi
fi
# ---------- PHP (hosted runtime) ----------
if has_target "php" "${NORMALIZED_TARGETS}"; then
echo "Emitting php (hosted runtime)"
"$PY_BIN" tools/ir_to_php.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/php/app
"$PY_BIN" tools/emit_output_manifest.py --root asm/php/app --manifest-out receipts/php_manifest.json
"$PY_BIN" tools/record_receipt.py --target receipts/php_manifest.json --receipt receipts/php.json --status CODEGEN_EMITTED_PHP --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/requirements.json tools/ir_to_php.py tools/emit_output_manifest.py tools/record_receipt.py --input-dirs spec asm --tool "$PY_BIN" --argv "$PY_BIN" tools/ir_to_php.py --variant hosted --ir-manifest receipts/ir_manifest.json --out-root asm/php/app --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
PHP_BIN=""
if [[ -f receipts/deps/php_runtime.json ]]; then
PHP_BIN="$("$PY_BIN" tools/dep_receipt_tool.py --receipt receipts/deps/php_runtime.json --require-accepted --print-path || true)"
fi
if [[ -n "$PHP_BIN" ]]; then
"$PHP_BIN" asm/php/app/main.php > asm/php/app/run_stdout.json
"$PY_BIN" tools/record_receipt.py --target asm/php/app/run_stdout.json --receipt receipts/php_run.json --status RUNTIME_STDOUT_EMITTED_PHP --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/php_manifest.json receipts/deps/php_runtime.json asm/php/app/main.php tools/dep_receipt_tool.py tools/record_receipt.py --input-dirs spec asm --tool "$PHP_BIN" --argv "$PHP_BIN" asm/php/app/main.php --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
else
"$PY_BIN" tools/record_receipt.py --target asm/php/app/run_stdout.json --receipt receipts/php_run.json --status SKIPPED_TOOLCHAIN --build-epoch "$STUNIR_BUILD_EPOCH" --epoch-json "$EPOCH_JSON" --inputs build/provenance.json receipts/ir_manifest.json receipts/ir_bundle_manifest.json receipts/php_manifest.json receipts/deps/php_runtime.json asm/php/app/main.php tools/dep_receipt_tool.py tools/record_receipt.py --input-dirs spec asm --exception-reason "${STUNIR_EPOCH_EXCEPTION_REASON:-}"
fi
fi
# ---------- Python (portable emission baseline) ----------
if has_target "python" "${NORMALIZED_TARGETS}"; then
echo "Emitting python (portable)"
"$PY_BIN" tools/ir_to_python.py \
--variant portable \
--ir-manifest receipts/ir_manifest.json \
...
PY_RUNTIME_BIN="$("$PY_BIN" tools/dep_receipt_tool.py --receipt receipts/deps/python_runtime.json --require-accepted --print-path || true)"
fi
...
Z3_BIN="$("$PY_BIN" tools/dep_receipt_tool.py --receipt receipts/deps/z3_solver.json --require-accepted --print-path || true)"
fi
if [[ -z "$Z3_BIN" ]]; then
