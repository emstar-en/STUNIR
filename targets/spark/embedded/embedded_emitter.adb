...
package body Embedded_Emitter is
   ...
   begin
         ...
         for I in 1 .. Func.Param_Count loop
            ...
            declare
               Param_Type : constant Type_Name_String := 
                  Get_C_Type (Func.Param_Types (I), Config);
            begin
               Append_To_Content (Content,
                  Type_Name_Strings.To_String (Param_Type) & " " &
                  Identifier_Strings.To_String (Func.Params (I)), Status);
            end;
         end loop;
      end if;
      ...
      --  Function body
      for I in 1 .. Func.Stmt_Count loop
         Emit_Statement (Func.Statements (I), Config, 4, Stmt_Content, Stmt_Status);
   ...
   begin
      ...
      for I in 1 .. Module.Func_Count loop
         declare
            Func : constant Embedded_Function := Module.Functions (I);
         ...
         begin
            ...
            if Func.Param_Count = 0 then
               ...
               for J in 1 .. Func.Param_Count loop
                  if J > 1 then
                     Append_To_Content (Content, ", ", Status);
                  end if;
                  declare
                     Param_Type : constant Type_Name_String :=
                        Get_C_Type (Func.Param_Types (J), Config);
                  begin
                     Append_To_Content (Content,
                        Type_Name_Strings.To_String (Param_Type) & " " &
                        Identifier_Strings.To_String (Func.Params (J)), Status);
                  end;
               end loop;
            end if;
   ...
   begin
      ...
      --  Emit all functions
      for I in 1 .. Module.Func_Count loop
         Emit_Function (Module.Functions (I), Config, Func_Content, Func_Status);
