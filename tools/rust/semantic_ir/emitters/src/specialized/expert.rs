//! STUNIR Expert Emitter - Rust Implementation
//!
//! Generates Expert system languages.
...
/// Expert emitter configuration
#[derive(Debug, Clone)]
pub struct ExpertConfig {
    /// Base emitter configuration
    pub base: EmitterConfig,
    /// Target variant
    pub variant: ExpertVariant,
}

impl ExpertConfig {
    /// Create new configuration
    pub fn new(base: EmitterConfig, variant: ExpertVariant) -> Self {
        Self { base, variant }
    }
}

/// Expert emitter
pub struct ExpertEmitter {
    config: ExpertConfig,
}
...
impl BaseEmitter for ExpertEmitter {
...
mod tests {
    ...
    #[test]
    fn test_expert_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = ExpertConfig::new(base_config, ExpertVariant::CLIPS);
        let emitter = ExpertEmitter::new(config);
