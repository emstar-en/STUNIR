//! STUNIR Asp Emitter - Rust Implementation
//!
//! Generates Answer Set Programming.
...
/// Asp emitter configuration
#[derive(Debug, Clone)]
pub struct AspConfig {
    /// Base emitter configuration
    pub base: EmitterConfig,
    /// Target variant
    pub variant: AspVariant,
}

impl AspConfig {
    /// Create new configuration
    pub fn new(base: EmitterConfig, variant: AspVariant) -> Self {
        Self { base, variant }
    }
}

/// Asp emitter
pub struct AspEmitter {
    config: AspConfig,
}

impl AspEmitter {
    /// Create new emitter
    pub fn new(config: AspConfig) -> Self {
        Self { config }
    }
...
impl BaseEmitter for AspEmitter {
...
mod tests {
    ...
    fn test_asp_emitter() {
        ...
        let config = AspConfig::new(base_config, AspVariant::Clingo);
        let emitter = AspEmitter::new(config);
