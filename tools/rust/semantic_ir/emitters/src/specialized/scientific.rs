//! STUNIR Scientific Emitter - Rust Implementation
//!
//! Generates Scientific computing languages.
...
/// Scientific emitter configuration
#[derive(Debug, Clone)]
pub struct ScientificConfig {
    /// Base emitter configuration
    pub base: EmitterConfig,
    /// Target variant
    pub variant: ScientificVariant,
}

impl ScientificConfig {
    /// Create new configuration
    pub fn new(base: EmitterConfig, variant: ScientificVariant) -> Self {
        Self { base, variant }
    }
}

/// Scientific emitter
pub struct ScientificEmitter {
    config: ScientificConfig,
}

impl ScientificEmitter {
    /// Create new emitter
    ...
    fn generate_code(&self, ir_module: &IRModule) -> Result<String, EmitterError> {
        ...
        writeln!(content, "# STUNIR Generated {:?} Code", self.config.variant)
            .map_err(|e| EmitterError::GenerationError(e.to_string()))?;
...
impl BaseEmitter for ScientificEmitter {
...
mod tests {
    ...
    #[test]
    fn test_scientific_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = ScientificConfig::new(base_config, ScientificVariant::MATLAB);
        let emitter = ScientificEmitter::new(config);
