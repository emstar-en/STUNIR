//! STUNIR Bytecode Emitter - Rust Implementation
//!
//! Generates Bytecode and IR formats.
...
/// Bytecode emitter configuration
#[derive(Debug, Clone)]
...
impl BytecodeConfig {
    ...
    pub fn new(base: EmitterConfig, variant: BytecodeVariant) -> Self {
        Self { base, variant }
    }
}
...
impl BaseEmitter for BytecodeEmitter {
...
mod tests {
    ...
    #[test]
    fn test_bytecode_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = BytecodeConfig::new(base_config, BytecodeVariant::JVM);
        let emitter = BytecodeEmitter::new(config);
