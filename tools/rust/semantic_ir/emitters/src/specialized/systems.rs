//! STUNIR Systems Emitter - Rust Implementation
//!
//! Generates Systems programming languages.
...
/// Systems emitter configuration
#[derive(Debug, Clone)]
pub struct SystemsConfig {
    /// Base emitter configuration
    pub base: EmitterConfig,
    /// Target variant
    pub variant: SystemsVariant,
}

impl SystemsConfig {
    /// Create new configuration
    pub fn new(base: EmitterConfig, variant: SystemsVariant) -> Self {
        Self { base, variant }
    }
}

/// Systems emitter
pub struct SystemsEmitter {
...
impl BaseEmitter for SystemsEmitter {
...
mod tests {
    ...
    #[test]
    fn test_systems_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = SystemsConfig::new(base_config, SystemsVariant::Ada);
        let emitter = SystemsEmitter::new(config);
