//! STUNIR Fpga Emitter - Rust Implementation
//!
//! Generates FPGA hardware description languages.
...
impl FpgaConfig {
    ...
    pub fn new(base: EmitterConfig, variant: FpgaVariant) -> Self {
        Self { base, variant }
    }
}
...
impl BaseEmitter for FpgaEmitter {
...
mod tests {
    ...
    #[test]
    fn test_fpga_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = FpgaConfig::new(base_config, FpgaVariant::VHDL);
        let emitter = FpgaEmitter::new(config);
