//! STUNIR Lexer Emitter - Rust Implementation
//!
//! Generates Lexer generators.
...
pub struct LexerConfig {
    ...
    /// Target variant
    pub variant: LexerVariant,
}

impl LexerConfig {
    /// Create new configuration
    pub fn new(base: EmitterConfig, variant: LexerVariant) -> Self {
        Self { base, variant }
    }
}

/// Lexer emitter
pub struct LexerEmitter {
    config: LexerConfig,
}
...
impl BaseEmitter for LexerEmitter {
...
mod tests {
    ...
    #[test]
    fn test_lexer_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = LexerConfig::new(base_config, LexerVariant::Flex);
        let emitter = LexerEmitter::new(config);
