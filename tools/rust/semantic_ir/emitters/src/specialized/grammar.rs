//! STUNIR Grammar Emitter - Rust Implementation
//!
//! Generates Grammar specification languages.
...
impl GrammarConfig {
    ...
    pub fn new(base: EmitterConfig, variant: GrammarVariant) -> Self {
        Self { base, variant }
    }
}
...
impl BaseEmitter for GrammarEmitter {
...
mod tests {
    ...
    #[test]
    fn test_grammar_emitter() {
        let temp_dir = TempDir::new().unwrap();
        let base_config = EmitterConfig::new(temp_dir.path(), "test_module");
        let config = GrammarConfig::new(base_config, GrammarVariant::ANTLR);
        let emitter = GrammarEmitter::new(config);
