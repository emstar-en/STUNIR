--  ir_check_functions - Validate function structures in IR
--  IR function validation utility for STUNIR powertools
...
procedure IR_Check_Functions is
   ...
   Describe_Output : constant String :=
     "{" & ASCII.LF &
     "  ""tool"": ""ir_check_functions""," & ASCII.LF &
     "  ""version"": ""0.1.0-alpha""," & ASCII.LF &
     "  ""description"": ""Validate function structures in IR""," & ASCII.LF &
     "  ""inputs"": [{" & ASCII.LF &
   ...
   function Check_Functions (IR : String) return Boolean;

   procedure Print_Usage is
   begin
      Put_Line ("ir_check_functions - Validate function structures in IR");
      Put_Line ("Version: " & Version);
      Put_Line ("");
      Put_Line ("Usage: ir_check_functions [OPTIONS]");
      Put_Line ("");
      Put_Line ("Options:");
   ...
   function Check_Functions (IR : String) return Boolean is
      Valid : Boolean := True;
   begin
      ...
      if IR'Length > 0 then
         ...
         begin
            for I in IR'First .. IR'Last - Functions_Pattern'Length + 1 loop
               if IR (I .. I + Functions_Pattern'Length - 1) = Functions_Pattern then
                  Found := True;
                  exit;
               end if;
            end loop;
            ...
            if not Found then
               Put_Line ("Missing 'functions' field");
               Valid := False;
            end if;
         end;
      end if;
...
begin
   ...
   if Show_Version then
      Put_Line ("ir_check_functions " & Version);
      return;
   end if;
