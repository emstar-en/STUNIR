--  rust_extract_main - Minimal Rust source -> extraction.json
...
procedure Rust_Extract_Main is
   ...
   Language    : Identifier_String := Identifier_Strings.To_Bounded_String ("rust");

   Exit_Failure : constant := 1;

begin
   if ACL.Argument_Count < 2 then
      Put_Line ("Usage: rust_extract_main -i <source> -o <extraction.json> [-m <module>] [--lang <lang>]");
...
exception
   when others =>
      Put_Line ("Error: rust_extract_main failed");
      ACL.Set_Exit_Status (Exit_Failure);
end Rust_Extract_Main;
