--  python_extract_main - Minimal Python source -> extraction.json
--  Phase 0: Source extraction (Python)
...
procedure Python_Extract_Main is
   ...
   Language    : Identifier_String := Identifier_Strings.To_Bounded_String ("python");

   Exit_Failure : constant := 1;

begin
   if ACL.Argument_Count < 2 then
      Put_Line ("Usage: python_extract_main -i <source> -o <extraction.json> [-m <module>] [--lang <lang>]");
...
exception
   when others =>
      Put_Line ("Error: python_extract_main failed");
      ACL.Set_Exit_Status (Exit_Failure);
end Python_Extract_Main;
