--  spec_extract_types - Extract types array from STUNIR spec JSON
--  Input:  Spec JSON (stdin)
--  Output: Types JSON array (stdout)
...
procedure Spec_Extract_Types is
   ...
   Describe_Output : constant String :=
     "{""tool"":""spec_extract_types"",""version"":""0.1.0-alpha""," &
...
begin
   ...
   if Show_Help then
      Put_Line ("spec_extract_types - Extract types array from STUNIR spec JSON");
      Put_Line ("Version: " & Version);
      New_Line;
      Put_Line ("Usage: spec_extract_types [OPTIONS] < spec.json");
      Put_Line ("  --module NAME   Extract from named module (default: first module)");
      Set_Exit_Status (Exit_Success); return;
   end if;
   if Show_Version then
      Put_Line ("spec_extract_types " & Version);
      Set_Exit_Status (Exit_Success); return;
   end if;
