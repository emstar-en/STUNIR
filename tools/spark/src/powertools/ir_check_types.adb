--  ir_check_types - Validate type definitions in IR
--  IR type validation utility for STUNIR powertools
...
procedure IR_Check_Types is
   ...
   Describe_Output : constant String :=
     "{" & ASCII.LF &
     "  ""tool"": ""ir_check_types""," & ASCII.LF &
     "  ""version"": ""0.1.0-alpha""," & ASCII.LF &
     "  ""description"": ""Validate type definitions in IR""," & ASCII.LF &
     "  ""inputs"": [{" & ASCII.LF &
   ...
   function Check_Types (IR : String) return Boolean;

   procedure Print_Usage is
   begin
      Put_Line ("ir_check_types - Validate type definitions in IR");
      Put_Line ("Version: " & Version);
      Put_Line ("");
      Put_Line ("Usage: ir_check_types [OPTIONS]");
      Put_Line ("");
      Put_Line ("Options:");
   ...
   function Check_Types (IR : String) return Boolean is
      Valid : Boolean := True;
   begin
      ...
      if IR'Length > 0 then
         ...
         begin
            for I in IR'First .. IR'Last - Types_Pattern'Length + 1 loop
               if IR (I .. I + Types_Pattern'Length - 1) = Types_Pattern then
                  Found := True;
                  exit;
               end if;
            end loop;
...
begin
   ...
   if Show_Version then
      Put_Line ("ir_check_types " & Version);
      return;
   end if;
