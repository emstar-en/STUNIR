                              ...
                              begin
                                 while Module.Functions (Func_Idx).Step_Count < Max_Steps loop
                                    ...
                                    declare
                                       ...
                                       Step_Index : constant String := Extract_String_Value (Step_JSON, "index");
                                       Step_Key   : constant String := Extract_String_Value (Step_JSON, "key");
                                       Step_Field : constant String := Extract_String_Value (Step_JSON, "field");
                                       Step_Source : constant String := Extract_String_Value (Step_JSON, "source");
                                    ...
                                    begin
                                       if Step_Op'Length > 0 then
                                          ...
                                          if Step_Index'Length > 0 then
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Index :=
                                               Name_Strings.To_Bounded_String (Step_Index);
                                          else
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Index :=
                                               Name_Strings.Null_Bounded_String;
                                          end if;

                                          if Step_Key'Length > 0 then
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Key :=
                                               Name_Strings.To_Bounded_String (Step_Key);
                                          else
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Key :=
                                               Name_Strings.Null_Bounded_String;
                                          end if;

                                          if Step_Field'Length > 0 then
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Field :=
                                               Name_Strings.To_Bounded_String (Step_Field);
                                          else
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Field :=
                                               Name_Strings.Null_Bounded_String;
                                          end if;

                                          if Step_Source'Length > 0 then
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Source :=
                                               Name_Strings.To_Bounded_String (Step_Source);
                                          else
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Source :=
                                               Name_Strings.Null_Bounded_String;
                                          end if;

                                          if Step_Source2'Length > 0 then
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Source2 :=
                                               Name_Strings.To_Bounded_String (Step_Source2);
                                          else
                                             Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Source2 :=
                                               Name_Strings.Null_Bounded_String;
                                          end if;

                                          Module.Functions (Func_Idx).Steps (Module.Functions (Func_Idx).Step_Count).Size := Step_Size;
      ...
      for I in 1 .. Step_Count loop
         if not Processed (I) then
            ...
            --  v0.8.8: Data structure operations - Arrays
               ...
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Size_Str : constant String := Name_Strings.To_String (Step.Value);
                  Elem_Type : constant String := "int32_t";
               begin
                  if Step.Size > 0 then
                     Append (Get_Indent & Elem_Type & " " & Target & "[" & Natural'Image (Step.Size) & "] = {0};");
                     Append (NL);
                  else
                     Append (Get_Indent & Elem_Type & "* " & Target & " = NULL;");
                     Append (NL);
                  ...
                  if Size_Str'Length > 0 then
                     Append (Get_Indent & "(void)" & Size_Str & ";");
                     Append (NL);
                  end if;
               end;
               ...
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Index  : constant String := Name_Strings.To_String (Step.Index);
               begin
                  Append (Get_Indent & "int32_t " & Target & " = " & Source & "[" & Index & "];");
                  Append (NL);
               end;
               ...
               declare
                  ...
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & Target & "[" & Index & "] = " & Value & ";");
                  Append (NL);
               end;

            elsif Op = "array_push" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & "if (" & Target & "_len >= " & Target & "_cap) {");
                  Append (NL);
                  Append (Get_Indent & "  " & Target & "_cap = " & Target & "_cap = 0 ? 8 : " & Target & "_cap * 2;");
                  Append (NL);
                  Append (Get_Indent & "  " & Target & " = realloc(" & Target & ", " & Target & "_cap * sizeof(" & Target & "[0]));");
                  Append (NL);
                  Append (Get_Indent & "}");
                  Append (NL);
                  Append (Get_Indent & Target & "[" & Target & "_len++] = " & Value & ";");
                  Append (NL);
               end;

            elsif Op = "array_pop" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
               begin
                  Append (Get_Indent & "int32_t " & Target & " = " & Source & "[--" & Source & "_len];");
                  Append (NL);
               end;

            elsif Op = "array_len" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
               begin
                  Append (Get_Indent & "size_t " & Target & " = " & Source & "_len;");
                  Append (NL);
               end;
               ...
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
               begin
                  Append (Get_Indent & "struct { void* key; void* value; bool used; }* " & Target & " = NULL;");
                  Append (NL);
                  Append (Get_Indent & "size_t " & Target & "_cap = 0;");
                  Append (NL);
               end;

            elsif Op = "map_get" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Key    : constant String := Name_Strings.To_String (Step.Key);
               begin
                  Append (Get_Indent & Target & " = stunir_map_get(" & Source & ", " & Source & "_cap, " & Key & ");");
                  Append (NL);
               end;

            elsif Op = "map_set" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Key    : constant String := Name_Strings.To_String (Step.Key);
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & "stunir_map_set(&" & Target & ", &" & Target & "_cap, " & Key & ", " & Value & ");");
                  Append (NL);
               end;

            elsif Op = "map_delete" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Key    : constant String := Name_Strings.To_String (Step.Key);
               begin
                  Append (Get_Indent & "stunir_map_delete(" & Target & ", " & Target & "_cap, " & Key & ");");
                  Append (NL);
               end;

            elsif Op = "map_has" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Key    : constant String := Name_Strings.To_String (Step.Key);
               begin
                  Append (Get_Indent & "bool " & Target & " = stunir_map_has(" & Source & ", " & Source & "_cap, " & Key & ");");
                  Append (NL);
               end;

            elsif Op = "map_keys" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
               begin
                  Append (Get_Indent & "size_t " & Target & "_len = stunir_map_keys(" & Source & ", " & Source & "_cap, &" & Target & ");");
                  Append (NL);
               end;
               ...
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
               begin
                  Append (Get_Indent & "struct { void* value; bool used; }* " & Target & " = NULL;");
                  Append (NL);
                  Append (Get_Indent & "size_t " & Target & "_cap = 0;");
                  Append (NL);
               end;

            elsif Op = "set_add" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & "stunir_set_add(&" & Target & ", &" & Target & "_cap, " & Value & ");");
                  Append (NL);
               end;

            elsif Op = "set_remove" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & "stunir_set_remove(" & Target & ", " & Target & "_cap, " & Value & ");");
                  Append (NL);
               end;

            elsif Op = "set_has" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & "bool " & Target & " = stunir_set_has(" & Source & ", " & Source & "_cap, " & Value & ");");
                  Append (NL);
               end;

            elsif Op = "set_union" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Source2 : constant String := Name_Strings.To_String (Step.Source2);
               begin
                  Append (Get_Indent & "stunir_set_union(" & Source & ", " & Source & "_cap, " & Source2 & ", " & Source2 & "_cap, &" & Target & ", &" & Target & "_cap);");
                  Append (NL);
               end;

            elsif Op = "set_intersect" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Source2 : constant String := Name_Strings.To_String (Step.Source2);
               begin
                  Append (Get_Indent & "stunir_set_intersect(" & Source & ", " & Source & "_cap, " & Source2 & ", " & Source2 & "_cap, &" & Target & ", &" & Target & "_cap);");
                  Append (NL);
               end;
               ...
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Struct_Type : constant String := Name_Strings.To_String (Step.Struct_Type);
               begin
                  if Struct_Type'Length > 0 then
                     Append (Get_Indent & "struct " & Struct_Type & " " & Target & " = {0};");
                  else
            ...
            elsif Op = "struct_get" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Source : constant String := Name_Strings.To_String (Step.Source);
                  Field  : constant String := Name_Strings.To_String (Step.Field);
               begin
                  Append (Get_Indent & "int32_t " & Target & " = " & Source & "." & Field & ";");
                  Append (NL);
               end;

            elsif Op = "struct_set" then
               declare
                  Target : constant String := Name_Strings.To_String (Step.Target);
                  Field  : constant String := Name_Strings.To_String (Step.Field);
                  Value  : constant String := Name_Strings.To_String (Step.Value);
               begin
                  Append (Get_Indent & Target & "." & Field & " = " & Value & ";");
                  Append (NL);
               end;
