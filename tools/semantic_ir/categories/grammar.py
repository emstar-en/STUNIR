...
class GrammarParser(CategoryParser):
    ...
    def validate_spec(self, spec: Dict) -> ValidationResult:
        """Validate grammar specification."""
        errors = []
        warnings = []
        return ValidationResult(is_valid=True, errors=errors, warnings=warnings)

    def build_category_ast(self, spec: Dict) -> CategoryAST:
        """Build grammar-specific AST."""
        nodes = [{"type": "grammar", "data": spec}]
        return CategoryAST(
